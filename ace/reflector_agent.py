...
from playbook import PlaybookEvaluation, PlaybookOperator
    ...
    def __init__(self, playbook_operator: PlaybookOperator):
        ...
        self.agent = Agent(
            ...
            instructions=(
                ...
                可以通过`grep`工具搜索Playbook中相关的策略条目。
                **分析要求：**
            ...
            tools=[
                playbook_operator.grep
            ]
        )

    async def reflect(self, objective: str, messages: list[ModelMessage]) -> Reflection:
        """
        反思智能体的执行轨迹，返回反思结果
        """
        # 构建反思提示
        ...
        response = await self.agent.run(
            user_prompt=f"根据此前的执行轨迹，请反思以下任务目标：{objective}",
